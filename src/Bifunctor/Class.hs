...
import Prelude hiding (Traversable (..), lcm, map)
...
wciBody :: forall (a :: Type). Char -> (M (State Bool) :.: Count) a
wciBody char =
  --  Comp . Wrap . fmap Const . state . updateState $ ch
  wrap (updateState char)
  where
    -- d :: (M (State Bool) :.: Const (Max Int)) a
    -- d = Comp c
    -- c :: M (State Bool) (Const (Max Int) a)
    -- c = Wrap b
    -- b :: State Bool (Const (Max Int) a)
    -- b = Const <$> a
    -- a :: State Bool (Max Int)
    -- a = state @Bool @(State Bool) (updateState ch)
    updateState :: Char -> Bool -> (Max Int, Bool)
    updateState ch inWord =
      let result = not (isSpace ch)
       in (test (not inWord && result), result)

wci :: forall (a :: Type). String -> (M (State Bool) :.: Count) [a]
wci = traverse wciBody

runWci :: String -> Max Int
runWci input = fst (run wci input False)

clci :: String -> (Count :*: Count) [a]
-- clci = cci <:*:> lci
clci = traverse (cciBody <:*:> lciBody)

clwci :: String -> ((Count :*: Count) :*: (M (State Bool) :.: Count)) [a]
...
wcqui :: String -> (Pair :*: (M (State Bool) :.: Count)) [Bool]
wcqui =
  -- qui <:*:> wci
  traverse (quiBody <:*:> wciBody)

wcqui' :: String -> ((Id :*: (M (State Bool) :.: Count)) :.: Pair) [Bool]
wcqui' = traverse (quiBody <:.:> (Id <:*:> wciBody))

ccmBody :: Char -> Writer (Max Int) Char
ccmBody ch = ch <$ tell 1

ccm :: String -> Writer (Max Int) String
ccm = mapM ccmBody

lcmBody :: Char -> Writer (Max Int) Char
lcmBody ch = ch <$ tell (test (ch == '\n'))

lcm :: String -> Writer (Max Int) String
lcm = mapM lcmBody

wcmBody :: Char -> State (Max Int, Bool) Char
wcmBody ch = do
  let result = not (isSpace ch)
  (n, w) <- get
  put (n + test (not w && result), result)
  pure ch

wcm :: String -> State (Max Int, Bool) String
wcm = mapM wcmBody

clwcm :: String -> ((Writer (Max Int) :*: Writer (Max Int)) :*: State (Max Int, Bool)) String
clwcm =
  -- ccm <:*:> lcm <:*:> wcm
  traverse (ccmBody <:*:> lcmBody <:*:> wcmBody)
