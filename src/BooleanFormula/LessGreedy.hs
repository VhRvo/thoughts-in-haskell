module BooleanFormula.LessGreedy where
...
satisfy expected env =
  \case
    ...
    And lhs rhs
      ...
      | otherwise -> do
        (satisfy True env lhs >>= \env' -> satisfy False env' rhs) <|>
          satisfy False env lhs
    Or lhs rhs
      | expected -> do
        (satisfy False env lhs >>= \env' -> satisfy True env' rhs) <|>
          satisfy True env lhs
      | otherwise -> do
